...
knitr::kable(party_incumbent_promote
             ...
             digits = 3)
```
## Democratic Incumbents promoted the most ads, on average.
...
plot_q3 <- ggplot(house_races, aes(x = impressions_millions)) +
  ...
  labs(title = "Distribution of Facebook Ad Impressions for US House Races",
...
## Most of the members of the House had under 10 million ad impressions. The result of this data being made into a histogram is therefore one where most House members fall close to the left-most end of the y-axis.
...
top_15_house <-
  top_15_house |>
  mutate(cand_name_last = factor(cand_name_last, levels = top_15_house$cand_name_last[order(top_15_house$impressions_millions)]))

fb_top_plot <- ggplot(top_15_house, aes(x = impressions_millions, y = cand_name_last)) +
  geom_bar(stat = "identity", aes(fill = party)) +
  labs(title = "Top 15 US House Candidates by Facebook Ad Impressions",
       x = "Ad Impressions (Millions)",
